...
export default function TratamentoOncologico({ pacienteId, nutriId }) {
  ...
  const [efeitoEscolhido, setEfeitoEscolhido] = useState(null);
  ...
  async function carregar(signal = { cancelled: false }) {
    const [{ data: trat }, { data: cic }, { data: ex }] = await Promise.all([
      ...
      supabase.from('ciclos_quimio').select('id,numero_ciclo,data_quimio,d3,d7,d10,d14,obs,aplicacao_no_ciclo').eq('paciente_id', pacienteId).order('data_quimio', { ascending: false }),
...
Retorne SOMENTE o JSON, sem nenhum texto antes ou depois.`;
  ...
  // de vazia: era o caso comum, e obrigava a nutri a caçar na lista o que já
  // estava no cadastro. Reaproveita o `proto` acima, que é o mesmo
  // getProtocolo() do texto livre do cadastro.
  //
  // `efeitoEscolhido` null significa que a nutri ainda não mexeu no <select>;
  // qualquer string é escolha dela e manda, inclusive voltar para o vazio de
  // "— selecione —". É derivação, não effect: preencher isso com setState
  // dentro de useEffect dispara render em cascata (e o eslint reclama).
  const efeitoProtocolo = efeitoEscolhido ?? proto?.nome ?? '';
  const protoEfeito = protocolosEfeitosData.protocolos
    .find(p => p.nome === efeitoProtocolo) ?? null;
      ...
      {secao === 'efeitos' && (
        <CardProtocoloEfeitos proto={protoEfeito}>
          ...
          <select
            ...
            onChange={e => setEfeitoEscolhido(e.target.value)}
